...
- **Date**: 2026-08-25 to 2026-08-30
- **Prompt**: A sequence: fix the EB-NeRD submission which showed "Submitted" with no score; then "run everything that is required in this assignment for ebnerd and minds and update the report"; then, after the course email stating that marks depend on rigorous engineering analysis (tool/index choices, impact on engineering metrics, comparison to alternatives, latency/throughput optimisations) rather than leaderboard rank, produce that analysis; finally produce the design note as LaTeX.
- **AI output used**: Yes, with two significant AI-originated conclusions later retracted on evidence (below).
- **Full-scale evaluation**: ran every required metric for both datasets. MIND entirely at full scale (val N=431,511): Q4 for both retrievers under both slicings, and recall@K for both. EB-NeRD semantic cold/warm at full scale (N=1,997,301); remaining EB-NeRD Q4 on a seeded 250,000 sample and BM25 recall@K on 20,000, because a full Q4 pass costs ~1h per retriever and full-corpus BM25 recall@K over 125,541 articles with ~267-article histories is multi-hour. **The sampling was validated rather than assumed**: the identical configuration measured both ways differs by 0.0003 AUC (0.5499 vs 0.5496), inside both bootstrap intervals. Added `--sample-n`/`--seed` to three CLIs so every sampled figure is reproducible from the command line.
- **Engineering analysis added** (new, in response to the course email): `src/evaluation/benchmark_index.py` compares the shipped exact `IndexFlatIP` against IVFFlat (nprobe 1/8/32) and HNSW (efSearch 32/128) on build time, on-disk size, single-query latency, p95, batch throughput, and recall measured *against the exact result*; `src/evaluation/benchmark_retrieval.py` measures each retriever's serving cost and every optimisation as a controlled before/after. Key numbers: IVF at nprobe=1 is 49x faster and returns 37% of true neighbours; targeted vs full-corpus candidate scoring is 27x (BM25) and 76x (semantic) for identical rankings; token memoisation is 5.1x.
- **AI reasoning that was wrong, caught by measurement (1)**: two optimisations proposed and implemented by the assistant — reusing a vocabulary-width IDF buffer to avoid an O(V) allocation, and vectorising the semantic candidate gather — were both **slower than the code they replaced** (0.938 vs 0.717 ms; 0.077 vs 0.062 ms). Both were argued from asymptotics and both were regressions: `np.zeros` is a C-level calloc with lazily-zeroed pages, so the memset was nearly free, while the Python loop added to *clear* the buffer was not. Rewriting both with NumPy fancy-indexing rather than Python loops gave the actual win (0.147 ms and 0.034 ms). Only benchmarking revealed this; the code comments now record the failed version alongside the working one.
- **AI reasoning that was wrong, caught by evidence (2)**: while no EB-NeRD submission had scored, the assistant found that the test set has 13,536,710 rows but only 13,336,711 distinct `impression_id`s (200,000 beyond-accuracy rows all carry id 0), noted that the course-provided `ebnerd_analysis.ipynb` emits one line per unique id, and inferred that our one-line-per-row file was the cause. A `--unique-ids` flag was built to match. **That inference was wrong** — the submission that later scored (0.5404) is the full row-aligned file exactly as originally generated. The symptom "submitted, no score" never discriminated between a malformed file and a slow evaluation queue, and the platform withholds scoring logs during the competition, so it could not have. The `--unique-ids` file was never submitted; dropping 200,000 rows would break the row-order alignment the guidelines require. Both `report.txt` and the design note record the retraction rather than quietly dropping the claim.
- **Q9 gap found and closed**: `tests/test_no_leakage.py`'s docstring claimed the suite asserted that `click_history` contains only pre-impression clicks, but no test touched `click_history`. Two real tests replaced the claim — EB-NeRD checked directly against article publication timestamps (5,437,152 entries, 0 violations), MIND checked structurally since it has no publication timestamps (history may only grow per user; 6,288 users, 0 violations). Suite: 9 passed, 1 correctly skipped.
- **Human modifications**: none to the code. The user directed scope throughout (assignment-only, sample-based verification, stop the full-scale EB-NeRD run when it became clear the slice was degenerate), performed all Codabench uploads, and captured the leaderboard screenshots.
- **Deliverables produced**: `design_note.tex` (LaTeX, compiles on a minimal BasicTeX install: avoids `enumitem`/`titlesec`, and a missing screenshot degrades to a visible placeholder rather than a blank section — the HTML draft had failed silently in exactly that way); `report.txt` rewritten around measured values with a Q1 section and contents map; README extended with the benchmarks, sampling flags and the prediction-file/gitignore reasoning.
- **Final results**: MIND semantic online 0.6497 (offline 0.6324), MIND BM25 online 0.5873; EB-NeRD semantic online 0.5404 (offline 0.5496). Both offline/online gaps under 0.02 with opposite signs and method ordering preserved, which is the substantive validation of the Q4 harness.
